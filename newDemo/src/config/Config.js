...
export const MTJF_PAGE_SIZE = 15;
...
export const reposDetailTopTabBarHeight = 60
...
/*二级页面专用 不然和一级页面一样顶部是吸附在上面的  二级页面需要自己加margin*/
export function getEntryPageStyle(Utils) {
    ...
    if (WXEnvironment.platform.toLowerCase() === 'ios') {
        if (Utils.env.isIPhoneX()) {
            ...
            return {height: mainHeight, width: '750px', marginTop: statusHeight + 4}
        }
    }
